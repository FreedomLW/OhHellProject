...
def action_id_from_teacher(choice, env: OhHellEnv2, state, action_mask) -> int:
    legal_positions = [idx for idx, allowed in enumerate(action_mask) if allowed]
    if not legal_positions:
        raise ValueError("Teacher was asked to act with no legal actions available")

    def _matches(decoded, target) -> bool:
        if isinstance(target, int) and isinstance(decoded, int):
            return decoded == target
        return decoded == target

    for pos in legal_positions:
        decoded = env._decode_action(int(pos), state)
        if _matches(decoded, choice):
            return int(pos)

    return int(legal_positions[0])
...
def collect_dataset(episodes: int, seed: int) -> List[StudentSample]:
    ...
    for episode in trange(episodes, desc="Collecting episodes"):
        ...
        while not done:
            ...
            action_id = action_id_from_teacher(teacher_choice, env, state, info["action_mask"])
